...
`], ...formatTraceEntry(entry));
  ...
  **/
  ...
  const getSelectionAndImage = async (maxResolution, antiMode) => {
    let result = null;
  ...
  const placeImageToSpecificDoc = async (base64Str, targetDocId, targetSelection, antiMode, layerType) => {
    ...
    await photoshop$1.core.executeAsModal(async () => {
      ...
      if (!targetDoc) throw new Error("目标文档已关闭");
      await photoshop$1.action.batchPlay(
        [
          {
            _obj: "select",
  ...
  const createGroupAndMask = async (layerIds, groupNamePrefix = "单图") => {
  ...
  const runSingleImage = async (options) => {
    ...
    if (successfulResults.length === 0) {
      ...
      throw new Error((firstError == null ? void 0 : firstError.errorMessage) || "所有生成任务均失败");
    }
    const createdLayerIds = [];
    const errorMessages = requestResults.filter((item) => !item.success).map((item) => `第 ${item.index} 张失败: ${item.errorMessage}`);
    ...
    for (const result of successfulResults) {
      try {
        ...
        if (typeof layerId === "number") {
          ...
          errorMessages.push(`第 ${result.index} 张回贴失败`);
        }
      } catch (error) {
        errorMessages.push(
          `第 ${result.index} 张回贴失败: ${error.message || "未知错误"}`
        );
      }
    }
  ...
  const getAiQuota = async (params) => {
    ...
    if (!apiKey) throw new Error("API Key 不能为空");
    if (!apiBaseUrl) throw new Error("API 地址不能为空");
    const response = await fetch(`${apiBaseUrl}/api/usage/token`, {
      method: "GET",
