...
`], ...formatTraceEntry(entry));
  ...
  **/
  ...
  const hostName$1 = uxp && ((_b = (_a = uxp == null ? void 0 : uxp.host) == null ? void 0 : _a.name) == null ? void 0 : _b.toLowerCase());
  const photoshop$1 = hostName$1 === "photoshop" ? require("photoshop") : {};
  hostName$1 === "indesign" ? require("indesign") : {};
  hostName$1 === "premierepro" ? require("premierepro") : {};
  hostName$1 === "illustrator" ? require("illustrator") : {};
  ...
  const callAiApi = async (apiKey, prompt, inputImageBase64, imageSize, timeoutSeconds, apiBaseUrl) => {
    ...
    try {
      ...
      const imagePart = (_d = (_c2 = (_b2 = (_a2 = data == null ? void 0 : data.candidates) == null ? void 0 : _a2[0]) == null ? void 0 : _b2.content) == null ? void 0 : _c2.parts) == null ? void 0 : _d.find(
        (part) => {
          var _a3;
  ...
  const placeImageToSpecificDoc = async (base64Str, targetDocId, targetSelection, antiMode, layerType) => {
    ...
    await photoshop$1.core.executeAsModal(async () => {
      ...
      const layer = (_b2 = photoshop$1.app.activeDocument.activeLayers) == null ? void 0 : _b2[0];
      createdLayerId = (layer == null ? void 0 : layer.id) ?? null;
    }, { commandName: "Place AI Result" });
    return createdLayerId;
  };
  const createGroupAndMask = async (layerIds, groupNamePrefix = "鍗曞浘") => {
  ...
  const normalizeBatchTaskItem = (task) => {
    ...
    const top = Math.round(toNumber((_b2 = task.selection) == null ? void 0 : _b2.top));
    const fallbackWidth = Math.round(toNumber((_c2 = task.selection) == null ? void 0 : _c2.right) - left);
  ...
  const runGlobalPartition = async (options) => {
    ...
    const docPlans = docs.map((doc2) => {
      ...
      return {
        docId: doc2.id,
        docName: String(doc2.name ?? `鏂囨。 ${doc2.id}`),
        width,
        height,
    ...
    for (const plan of docPlans) {
      ...
      try {
        ...
        const message = `鍒囨崲鏂囨。澶辫触: ${error.message || "鏈煡閿欒"}`;
      ...
      for (const selection of plan.selections) {
        try {
          ...
          const message = `鍒嗗尯 ${selection.name} 鎶撳彇寮傚父: ${error.message || "鏈煡閿欒"}`;
          docErrors.push(message);
          errorMessages.push(`[${plan.docName}] ${message}`);
          docFailureCount += batchSize;
        }
      ...
      for (const capture of captures) {
          ...
          Array.from({ length: batchSize }, async (_, index) => {
            try {
              ...
              return {
                index: index + 1,
                success: false,
                errorMessage: error.message || "鐢熸垚澶辫触"
              };
            }
          })
        );
        ...
        if (createdLayerIds.length > 0) {
          try {
            ...
            const message = `鍒嗗尯 ${capture.selection.name} 鍒嗙粍澶辫触: ${error.message || "鏈煡閿欒"}`;
            docErrors.push(message);
            errorMessages.push(`[${plan.docName}] ${message}`);
          }
        }
    ...
    for (const task of tasks) {
      ...
      if (createdLayerIds.length > 0) {
        try {
          ...
          groupErrors.push(`鍒嗙粍澶辫触: ${error.message || "鏈煡閿欒"}`);
        }
      }
      await deselectAll();
  ...
  const runSingleImage = async (options) => {
    ...
    if (successfulResults.length === 0) {
      ...
      throw new Error((firstError == null ? void 0 : firstError.errorMessage) || "鎵€鏈夌敓鎴愪换鍔″潎澶辫触");
    }
    const createdLayerIds = [];
  ...
  const hostName = ((_c = uxp == null ? void 0 : uxp.host) == null ? void 0 : _c.name.toLowerCase().replace(/\s/g, "")) || "";
  let host = {};
  if (hostName.includes("photoshop") || hostName === "ps") host = photoshop;
  const api = { ...uxpLib, ...host };
